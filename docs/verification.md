...
* `go test -race -cover ./...`: passed. Core statement coverage **89.0%**, CLI **71.4%**. Includes all six provider success paths, source fallback, duration/version matching, request limits, cancellation, status/encoding failures, secret redaction, concurrent searches and concurrent temporary-token access.
...
* Python wrapper: **15 tests passed**, including a wheel installed into a clean Python 3.13 environment. Initial CI detected an empty wheel caused by an inherited ignore rule; explicit package selection fixed it, and both Python 3.11/3.13 installed-wheel CI checks passed. Ruff **0.16.8** lint/format passed.
